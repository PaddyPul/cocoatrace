...
export function FarmsPage() {
  ...
  return <Layout currentPage="farms">
    {loading ? <SkeletonTable rows={5} cols={6} /> : error ? <Err msg={error} /> : <div className="table-wrap">
      ...
      <table><thead><tr><th>Name</th><th>Region</th><th>Country</th><th>District</th><th>COCOBOD ID</th><th>Status</th></tr></thead><tbody>{data.length > 0 ? (data as Farm[]).map((f) => <tr key={f.id} className="cursor-pointer hover:bg-brand-500/5" onClick={() => navigate(`/farms/${f.id}`)}><td className="text-text-primary font-medium">{f.name}</td><td>{f.region}</td><td>{f.country}</td><td>{f.district}</td><td className="font-mono text-[11px]">{f.official_traceability_id || '—'}</td><td><StatusBadge status={f.verification_status} /></td></tr>) : <tr><td colSpan={99}><EmptyState icon="🏡" title="No farms registered" description="Register your first farm to start tracing cocoa from plot to port." action={canDo('farm.create') ? <button className="btn btn-sm btn-primary" onClick={() => setShowCreate(true)}>+ Create Farm</button> : undefined} /></td></tr>}</tbody></table></div>}
...
export function BatchesPage() {
  ...
  return <Layout currentPage="batches">
    {loading ? <SkeletonTable rows={5} cols={7} /> : error ? <Err msg={error} /> : <div className="table-wrap">
      ...
      <table><thead><tr><th>Batch ID</th><th>Farm</th><th>Harvest Date</th><th>Qty (kg)</th><th>Grade</th><th>Status</th><th></th></tr></thead><tbody>{batches.length > 0 ? batches.map((b) => <tr key={b.id} className="cursor-pointer hover:bg-brand-500/5" onClick={() => navigate(`/batches/${b.id}`)}><td className="font-mono text-[11px]">{b.id.slice(0, 13)}…</td><td className="text-text-primary font-medium">{b.farm_name || '—'}</td><td>{fmtDate(b.harvest_date)}</td><td>{(b.quantity_kg || 0).toLocaleString()}</td><td>{b.grade || '—'}</td><td><StatusBadge status={b.organic_claim_status} /></td><td>{b.current_holder_id === user?.organizationId && canDo('batch.create') ? <span className="badge badge-green font-mono text-[10px]">You hold this</span> : <span className="text-text-muted text-[10px]">Details →</span>}</td></tr>) : <tr><td colSpan={99}><EmptyState icon="📦" title="No batches yet" description="Create a batch after harvesting to begin the traceability chain." action={canDo('batch.create') ? <button className="btn btn-sm btn-primary" onClick={() => setShowCreate(true)}>+ Create Batch</button> : undefined} /></td></tr>}</tbody></table></div>}
...
export function HoldingsPage() {
  ...
  return <Layout currentPage="holdings">{loading ? <SkeletonTable rows={5} cols={6} /> : error ? <Err msg={error} /> : <div className="table-wrap"><table><thead><tr><th>ID</th><th>Farm</th><th>Crop</th><th>Qty (kg)</th><th>Warehouse</th><th>Status</th></tr></thead><tbody>{(data as Holding[]).length > 0 ? (data as Holding[]).map((h) => <tr key={h.id} className="cursor-pointer hover:bg-brand-500/5" onClick={() => navigate(`/holdings/${h.id}`)}><td className="font-mono text-[11px]">{h.id.slice(0, 13)}…</td><td className="text-text-primary font-medium">{h.farm_name || '—'}</td><td>{h.crop || 'cocoa'}</td><td>{(h.quantity_kg || 0).toLocaleString()}</td><td className="text-[11px]">{h.warehouse_location || '—'}</td><td><StatusBadge status={h.status} /></td></tr>) : <tr><td colSpan={99}><EmptyState icon="🏪" title="No holdings yet" description="Holdings represent batch inventory in your custody. They appear when batches are created or transferred to your organization." /></td></tr>}</tbody></table></div>}</Layout>;
}

export function ContractsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useFetch(() => contractsApi.list());
  return <Layout currentPage="contracts">{loading ? <SkeletonTable rows={5} cols={7} /> : error ? <Err msg={error} /> : <div className="table-wrap"><table><thead><tr><th>Contract</th><th>Seller</th><th>Buyer</th><th>Qty (kg)</th><th>Value</th><th>Incoterm</th><th>Status</th></tr></thead><tbody>{(data as Contract[]).length > 0 ? (data as Contract[]).map((c) => <tr key={c.id} className="cursor-pointer hover:bg-brand-500/5" onClick={() => navigate(`/contracts/${c.id}`)}><td className="font-mono text-[11px]">{c.id.slice(0, 8)}…</td><td>{c.seller_name || '—'}</td><td className="text-text-primary font-medium">{c.buyer_name || '—'}</td><td>{(c.quantity_kg || 0).toLocaleString()}</td><td className="text-brand-400">€{Math.round(c.quantity_kg * c.price_per_kg).toLocaleString()}</td><td className="font-mono text-[11px]">{c.incoterm}</td><td><StatusBadge status={c.status} /></td></tr>) : <tr><td colSpan={99}><EmptyState icon="📄" title="No contracts yet" description="Contracts are created automatically when a seller accepts an offer on the marketplace." action={<button className="btn btn-sm" onClick={() => navigate('/marketplace')}>Browse Marketplace →</button>} /></td></tr>}</tbody></table></div>}</Layout>;
}

export function ShipmentsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useFetch(() => shipmentsApi.list());
  return <Layout currentPage="shipments">{loading ? <SkeletonTable rows={5} cols={6} /> : error ? <Err msg={error} /> : <div className="table-wrap"><table><thead><tr><th>Vessel</th><th>Container</th><th>Route</th><th>ETA</th><th>B/L</th><th>Milestone</th></tr></thead><tbody>{(data as Shipment[]).length > 0 ? (data as Shipment[]).map((s) => <tr key={s.id} className="cursor-pointer hover:bg-brand-500/5" onClick={() => navigate(`/shipments/${s.id}`)}><td className="text-text-primary font-medium">{s.vessel_name || 'Shipment'}</td><td className="font-mono text-[11px]">{s.container_reference || '—'}</td><td className="text-[11px]">{s.origin_port} → {s.destination_port}</td><td>{fmtDate(s.eta_arrival)}</td><td className="font-mono text-[11px]">{s.bill_of_lading_number || '—'}</td><td><StatusBadge status={s.current_milestone} /></td></tr>) : <tr><td colSpan={99}><EmptyState icon="🚢" title="No shipments yet" description="Shipments are created when a seller requests logistics for a contract." /></td></tr>}</tbody></table></div>}</Layout>;
}

export function PaymentsPage() {
  const navigate = useNavigate();
  const { data, loading, error } = useFetch(() => paymentsApi.list());
  return <Layout currentPage="payments">{loading ? <SkeletonTable rows={5} cols={6} /> : error ? <Err msg={error} /> : <div className="table-wrap"><table><thead><tr><th>Payment ID</th><th>Contract</th><th>Amount</th><th>Currency</th><th>Status</th><th>Reference</th></tr></thead><tbody>{(data as Payment[]).length > 0 ? (data as Payment[]).map((p) => <tr key={p.id} className="cursor-pointer hover:bg-brand-500/5" onClick={() => navigate(`/payments/${p.id}`)}><td className="font-mono text-[11px]">{p.id.slice(0, 8)}…</td><td className="font-mono text-[11px]">{p.contract_id.slice(0, 8)}…</td><td className="text-brand-400 font-mono">{fmtMoney(p.amount_total, p.currency)}</td><td>{p.currency}</td><td><StatusBadge status={p.status} /></td><td className="font-mono text-[10px]">{p.payment_reference_external || '—'}</td></tr>) : <tr><td colSpan={99}><EmptyState icon="💰" title="No payments yet" description="Payment requests appear when a seller requests payment for a contract." /></td></tr>}</tbody></table></div>}</Layout>;
}

export function EvidencePage() {
  const { data, loading, error } = useFetch(() => evidenceApi.list());
  return <Layout currentPage="evidence">{loading ? <SkeletonTable rows={5} cols={6} /> : error ? <Err msg={error} /> : <div className="table-wrap"><table><thead><tr><th>File</th><th>Type</th><th>SHA-256</th><th>Linked Entity</th><th>Status</th><th>Uploaded</th></tr></thead><tbody>{(data as Evidence[]).length > 0 ? (data as Evidence[]).map((e) => <tr key={e.id} className="hover:bg-brand-500/5"><td className="text-text-primary font-medium">{e.file_name}</td><td>{(e.type || '').replace(/_/g, ' ')}</td><td className="font-mono text-[10px] max-w-[200px] truncate">{e.sha256_hash}</td><td className="font-mono text-[10px]">{e.linked_entity_type}/{e.linked_entity_id.slice(0, 8)}…</td><td><StatusBadge status={e.review_status} /></td><td className="text-[11px]">{fmtDate(e.created_at)}</td></tr>) : <tr><td colSpan={99}><EmptyState icon="🗂" title="No evidence uploaded" description="Evidence documents support traceability claims and certification audits." /></td></tr>}</tbody></table></div>}</Layout>;
}
...
export function AuditPage() {
  ...
  })}{data.length === 0 && <tr><td colSpan={99}><EmptyState icon="🔗" title="No audit events" description="Audit events are recorded automatically for all platform actions." /></td></tr>}</tbody></table></div>}</Layout>;
}
