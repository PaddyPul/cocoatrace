  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-surface-darker">
      <div className="bg-surface-dark border border-border rounded-xl p-10 w-full max-w-sm">
        ...
        {error && (
          <div className="bg-red-900/10 border border-red-500/30 rounded-sm px-3 py-2 mb-4 text-xs text-red-400">{error}</div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn btn-primary w-full justify-center mt-1" disabled={submitting}>
            {submitting ? 'Signing in…' : 'Sign in'}
          </button>
        </form>

        <div className="mt-6 pt-5 border-t border-border/50">
          <div className="text-[10px] text-text-muted uppercase tracking-wider mb-2">Quick logins (all: Password123!)</div>
          <div className="flex flex-wrap gap-1.5">
            {QUICK_USERS.map((u) => (
              <button key={u.email} className="px-2.5 py-1 bg-surface-darker border border-border-strong rounded-full text-[11px] text-text-secondary cursor-pointer hover:bg-brand-500/10 hover:border-brand-500 hover:text-brand-400 transition-all" onClick={() => quickLogin(u.email)}>
                {u.label}
              </button>
            ))}
          </div>
