...
router.get('/provenance/batches/:batchId', requireAuth, async (req, res) => {
...
router.get('/provenance/batches/:batchId/export', requireAuth, async (req, res) => {
  const { batchId } = req.params;
  const { contractId, format = 'json' } = req.query;

  const baseUrl = `${req.protocol}://${req.get('host')}`;
  const url = new URL(`${baseUrl}/provenance/batches/${batchId}`);
  if (contractId) url.searchParams.set('contractId', contractId);

  // Reuse the same DB logic by querying directly through the existing endpoint logic later.
  // For V1, duplicate the same response shape from the main provenance route.
  const [batchRes, farmRes] = await Promise.all([
    query(`SELECT b.*, f.name as farm_name, f.region, f.country, f.official_traceability_id,
                  a.attested_at, a.provenance_hash as att_hash,
                  c.standard, c.valid_from, c.valid_to, cert_org.name as certifier_name
  ...
  const evidenceRes = await query(
    "SELECT * FROM evidence_items WHERE linked_entity_type='batch' AND linked_entity_id=$1",
    [batchId]
  );
  ...
  const policyChecks = [
    { rule: 'Batch has organic attestation', passed: !!batch.attestation_id, warning: false },
    {
      ...
      passed: !!batch.attestation_id &&
        new Date(batch.harvest_date) >= new Date(batch.valid_from) &&
        new Date(batch.harvest_date) <= new Date(batch.valid_to),
      warning: false,
    },
    { rule: 'Plot geolocation present', passed: plots.some(p => p.gps_lat), warning: !plots.some(p => p.gps_lat) },
    { rule: 'EUDR cutoff checked', passed: plots.every(p => p.eudr_cutoff_checked), warning: !plots.every(p => p.eudr_cutoff_checked) },
  ...
  const completenessChecks = [
    !!batch.attestation_id,
    plots.length > 0,
    plots.some(p => p.gps_lat),
    evidenceRes.rows.some(e => e.type === 'certificate_pdf'),
    evidenceRes.rows.some(e => e.type === 'weighing_ticket'),
  ...
  const exportPayload = {
    ...
    eudrReadiness: {
      ...
      ready:
        plots.some(p => p.gps_lat) &&
        plots.every(p => p.eudr_cutoff_checked) &&
        !!contract?.eudr_due_diligence_reference,
    },
  };
  ...
  await audit.record({
    ...
    action: 'provenance.export',
    entityType: 'harvest_batch',
    entityId: batchId,
    reason: `Exported provenance pack as ${format}`,
  });
  ...
  if (format === 'json') {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="provenance-pack-${batchId}.json"`
    );
    return res.json(exportPayload);
  }

  return res.status(400).json({ error: 'Unsupported export format. Use ?format=json' });
});
